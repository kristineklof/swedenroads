#=================================================================#
#           Descriptove statistics for 2023 update report
...
setDT(swedt_PCI)
#pci2032 <- read.xlsx("C:/Users/winte/Swedenroads_outputs/2030_PCI_20201218.xlsx")
...
#pci2032_2022 <- fread("C:/Users/krist/OneDrive - Salbo Konsult AB/salbo.ai/Swedenroads_slutversioner/7018-Scenario6.csv")
...
#Mean PCI 2032
means <- pci2032 %>% group_by(Year) %>%
  summarize(mean_pci = mean(PCI))

#names(pci2032_2022) <- c("Objectd","Year","PCI")
#setDT(pci2032_2022)
...
pci2032 <- PCIClass2030(pci2032)
#pci2032_2022 <- PCIClass2030(pci2032_2022)
#pci2032[, PCIClass_2030 := as.factor(PCIClass)]
#pci2032[, PCIClass := NULL]
str(pci2032)
head(pci2032)
...
pci2032 <- pci2032[swedt_PCI[, ..cols], on = 'Objectd']
#pci2032 <- pci2032_2022[pci2032, on = c('Objectd','Year')]

fill <- c("#20AC65", "#71C94B","#FABF20","#F2203E","#C40A3B")

###############################################################################
# PCI 2020 vs 2030 barchart traffic
#df_long <- melt(data = pci2032, 
#                id.vars = c("Objectd","AADT","Length"),
#                measure.vars = c("PCIClass_2030", "PCIClass"),
#                variable.name = "Y",
#                value.name = "PCIClass")

df_long <- pci2032[Year == 2022 | Year == 2032]
df_long[, Y := Year]
...
cond_y <- df_long %>%
  ...
  mutate(Y = factor(Y, levels = c('2022', '2032'))) %>%
  mutate(PCIClass = factor(PCIClass, levels = c("5","4","3","2","1"))) %>%
  mutate(PCIClass = recode(PCIClass, "5" ="Mycket bra", "4" = "Bra", "3" = "Tillfredsställande", "2" = "Dålig", "1" = "Mycket dålig")) %>%
  group_by(Y, PCIClass) %>%
...
#####################################################
# Plot tillstånd over time 2022-2032
...
def_df_urek <- data.frame(Year = c(2022,2023,2024,2025,2026,2027,2028,2029,2030,2031,2032),
                          Miljarder = c(10.6, 13.2, 16.6, 22.2, 28.4, 25.9, 28.3, 31.0, 34.3, 37.7, 42.0))
