#=================================================================#
#                  Update 2023 data functions
#=================================================================#

UpdateVariables23 <- function(sweden_yy, lans_dt){
  # Add beläggning 2023
  sw_yy <- sweden_yy %>% dplyr::mutate(Treatment = beltyp_23)
  sw_yy <- AddPavementType(sw_yy, lans_dt)
  sw_yy[, Treatment := NULL]
  sw_yy <- sw_yy %>% dplyr::mutate(PavementType = as.character(PavementType)) %>% 
    dplyr::mutate(PavementType = if_else_na(beltyp_23 == "MJOG - Mjukbitumenbundet grus med oljegrusgradering",
                                          "Halvvarm",PavementType)) %>% 
     dplyr::mutate(PavementType = as.factor(PavementType)) 
  
  # Add ÅDT mätår 202x
  sw_yy <- sw_yy %>% dplyr::mutate(adt_mat_23 = substr(as.character(adt_mat_23), 1, 4)) %>%
    dplyr::mutate(adt_mat_23 = as.numeric(adt_mat_23))
  
  # Uppdatera NVDB variabler
  sw_yy <- sw_yy %>% 
    dplyr::mutate(brghtsk = if_else_na(brghtsk != barig_23, barig_23, brghtsk)) %>%
    #dplyr::mutate(hastght = if_else_na(hastght != hast_23, hast_23, hastght)) %>%
    dplyr::mutate(dou2017 = if_else_na(dou2017 != dou_23, dou_23, dou2017)) %>%
    dplyr::mutate(ådt_frd = if_else_na(ådt_frd !=  adt_23,  adt_23, ådt_frd)) %>%
    dplyr::mutate(ådt_tng = if_else_na(ådt_tng != adt_tung_2, adt_tung_2, ådt_tng)) %>%
    dplyr::mutate(ådt_mtr = if_else_na(ådt_mtr != adt_mat_23, adt_mat_23, ådt_mtr)) %>%
    dplyr::mutate(vägbrdd = if_else_na(vägbrdd != vagbredd_2, vagbredd_2, vägbrdd)) %>%
    dplyr::mutate(vägtyp = if_else_na(vägtyp != vagtyp_23, vagtyp_23, vägtyp))
  
  # Uppdatera trafikklass
  sw_yy <- TrafficClass2023(sw_yy)
  
  return(sw_yy)
}

NewLifetime23 <- function(sw_yy, nvdb_2020, lan_surv_dt){
  # Uppdatera ålder
  sw_yy <- sw_yy %>% dplyr::mutate(blggnngsd = UpdateBelaggningsdatum(blggnngsd, beldat_23)) %>%
...
CalculatePCI23 <- function(sw_yy){
  pci_data <- sw_yy %>% dplyr::select(id, PredictedServiceLife, RemainingServiceLife,
                                      IRI_maint, SP_maint, sparm17_23, sparm15_23, irih_23, 
                                      blggnngsd, mätdatm, RoadType, vägbrdd, trfkkls) %>%
    dplyr::rename(IRI_r_p = irih_23) %>%
    dplyr::rename(IRI_mnt = IRI_maint) %>%
    dplyr::rename(rt_m17_ = sparm17_23) %>%
    dplyr::rename(rt_m15_ = sparm15_23) %>%
  ...
  pci_data <- pci_data %>%
    dplyr::rename(IRI_Index_23 = IRI_Index) %>%
    dplyr::rename(Rut_17_Index_23 = Rut_17_Index) %>%
    dplyr::rename(Rut_15_Index_23 = Rut_15_Index) %>%
    dplyr::rename(RMS_Index_23 = RMS_Index) %>%
    dplyr::rename(Rut_Index_23 = Rut_Index) %>%
    dplyr::rename(PCI_23 = PCI) %>%
    dplyr::rename(PCIClass_23 = PCIClass) 
  
  pci_data <- pci_data %>% dplyr::select(id, IRI_Index_23,
                                         Rut_17_Index_23, Rut_15_Index_23,
                                         RMS_Index_23, Rut_Index_23,
                                         PCI_23, PCIClass_23)
