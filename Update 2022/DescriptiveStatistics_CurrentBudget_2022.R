#=================================================================#
#           Descriptove statistics for 2022 update report
...
#pci2030_2020 <- fread("C:/Users/krist/OneDrive - Salbo Konsult AB/salbo.ai/Swedenroads_slutversioner/7018-Scenario6.csv")
...
pci2030_2020 <- PCIClass2030(pci2030_2020)
...
#pci2030 <- pci2030_2020[pci2030, on = c('Objectd','Year')]
...
df_long <- pci2030[Year == 2021 | Year == 2031]
df_long[, Y := Year]
head(df_long)
...
cond_y <- df_long %>%
  ...
  mutate(Y = factor(Y, levels = c('2021', '2031'))) %>%
  mutate(PCIClass = factor(PCIClass, levels = c("5","4","3","2","1"))) %>%
  mutate(PCIClass = recode(PCIClass, "5" ="Mycket bra", "4" = "Bra", "3" = "Tillfredsställande", "2" = "Dålig", "1" = "Mycket dålig")) %>%
  group_by(Y, PCIClass) %>%
...
#####################################################
# Plot tillstånd over time 2021-2031
...
def_df_urek <- data.frame(Year = c(2021,2022,2023,2024,2025,2026,2027,2028,2029,2030,2031),
                          Miljarder = c(11.3, 13.7, 16.7, 21.6, 26.8, 31.4, 35.8, 40.1, 44.6, 49.2, 54.3))
...
def_b_rek <- def_df_urek %>%
  ...
  geom_bar(position="dodge", stat="identity", color="black", fill="darkblue") +
  geom_text(aes(label=Miljarder), position=position_dodge(width=0.9), vjust=-0.25, fontface="bold", size=8) +
  scale_y_continuous(name="Miljarder SEK", limits=c(0, 60), breaks=c(0,10,20,30,40,50,60)) +
  #geom_text(aes(label=Miljarder), size=10) +
  labs(y="Miljarder SEK", x = "År") +
  theme(axis.text=element_text(size=24),
        axis.title=element_text(size=24))
