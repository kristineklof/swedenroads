...
pci2030_mcur <- fread("C:/Users/krist/OneDrive - Salbo Konsult AB/salbo.ai/Swedenroads_slutversioner/2022/Scenarios/2022 Scenario 4E_CI.csv")
...
df_long <- pci2030_mcur[Year == 2021 | Year == 2031]
df_long[, Y := Year]
head(df_long)
...
def_df_maint_current <- data.frame(Year = c(2021,2022,2023,2024,2025,2026,2027,2028,2029,2030,2031),
                                   Miljarder = c(11.3, 13.0, 11.7, 9.4, 9.0, 9.6, 9.9, 7.9, 10.0, 11.9, 14.1))
...
################################################################################
# Plot budget
bud_df_maint_current <- data.frame(Year = c(2022,2023,2024,2025,2026,2027,2028,2029,2030,2031),
                                   Miljarder = c(3.2, 20.8, 5.3, 6.5, 4.6, 3.8, 4.4, 0.4, 0.5, 0.5))
h <- 5.3
